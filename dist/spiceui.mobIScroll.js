...
var ms = $.spiceMobIScroll,
    ...
    var s = $.extend({}, defaults, html5def, inst.settings),
        ...
        p = s.preset,
        dord = s[s.i18n].dateOrder,
        tord = s[s.i18n].timeWheels,
        regen = dord.match(/D/),
        ampm = tord.match(/a/i),
        hampm = tord.match(/h/),
        hformat = p == 'datetime' ? s[s.i18n].dateFormat + s.separator + s[s.i18n].timeFormat : p == 'time' ? s[s.i18n].timeFormat : s[s.i18n].dateFormat,
        defd = new Date(),
        stepH = s.stepHour,
        stepM = s.stepMinute,
        stepS = s.stepSecond,
    ...
    if (p.match(/date/i)) {
        ...
        for (k = 0; k < 3; k++) {
            if (k == o.y) {
                ...
                for (i = start; i <= end; i++) {
                    w[s[s.i18n].yearText][i] = dord.match(/yy/i) ? i : (i + '').substr(2, 2) /* + s[s.i18n].yearText*/;
                }
            } else if (k == o.m) {
                offset++;
                w[s[s.i18n].monthText] = {};
                for (i = 0; i < 12; i++) {
                    var str = dord.replace(/[dy]/gi, '').replace(/mm/, i < 9 ? '0' + (i + 1) : i + 1).replace(/m/, i + 1);
                    w[s[s.i18n].monthText][i] = str.match(/MM/) ? str.replace(/MM/, '<span class="dw-mon">' + s[s.i18n].monthNames[i] + '</span>') : str.replace(/M/, '<span class="dw-mon">' + s[s.i18n].monthNamesShort[i] + '</span>') /* + s[s.i18n].monthText*/;
                }
            } else if (k == o.d) {
                offset++;
                w[s[s.i18n].dayText] = {};
                for (i = 1; i < 32; i++) {
                    w[s[s.i18n].dayText][i] = dord.match(/dd/i) && i < 10 ? '0' + i : i /* + s[s.i18n].dayText*/;
                }
            }
        }
        wheels.push(w);
        ...
        for (k = offset; k < offset + 4; k++) {
            if (k == o.h) {
                ...
                for (i = 0; i < (hampm ? 12 : 24); i += stepH) {
                    w[s[s.i18n].hourText][i] = hampm && i == 0 ? 12 : tord.match(/hh/i) && i < 10 ? '0' + i : i;
                }
            } else if (k == o.i) {
                offset++;
                w[s[s.i18n].minuteText] = {};
                for (i = 0; i < 60; i += stepM) {
                    w[s[s.i18n].minuteText][i] = tord.match(/ii/) && i < 10 ? '0' + i : i;
                }
            } else if (k == o.s) {
                offset++;
                w[s[s.i18n].secText] = {};
                for (i = 0; i < 60; i += stepS) {
                    w[s[s.i18n].secText][i] = tord.match(/ss/) && i < 10 ? '0' + i : i;
                }
            } else if (k == o.a) {
                offset++;
    ...
    return {
        ...
        headerText: function headerText(v) {
        ...
        */
        validate: function validate(dw, i) {
            var temp = inst.temp,
                //.slice(0),
                    ...
                    if (i == 'd') {
                        ...
                        if (regen) {
                            $('.dw-li', t).each(function () {
                                ...
                                $('.dw-i', that).html(str.match(/DD/) ? str.replace(/DD/, '<span class="dw-day">' + s[s.i18n].dayNames[w] + '</span>') : str.replace(/D/, '<span class="dw-day">' + s[s.i18n].dayNamesShort[w] + '</span>'));
                            });
                        }
                    }
                    if (minprop && mind) {
        ...
        methods: {
            ...
            * @return {Date}
            */
            getDate: function getDate(temp) {
                var inst = $(this).spiceMobIScroll('getInst');
                if (inst) {
            ...
            */
            setDate: function setDate(d, fill, time, temp) {
                if (fill == undefined) {
                    fill = false;
                }
        ...
        if (literal) {
            ...
            switch (format.charAt(i)) {
                case 'd':
                    ...
                    break;
                case 'D':
                    output += f2('D', date.getDay(), s[s.i18n].dayNamesShort, s[s.i18n].dayNames);
                    break;
                case 'o':
                ...
                case 'm':
                    ...
                    break;
                case 'M':
                    output += f2('M', date.getMonth(), s[s.i18n].monthNamesShort, s[s.i18n].monthNames);
                    break;
                case 'y':
        ...
        if (literal) {
            ...
            switch (format.charAt(iFormat)) {
                ...
                case 'm':
                    ...
                    break;
                case 'M':
                    month = getName('M', s[s.i18n].monthNamesShort, s[s.i18n].monthNames);
                    break;
                case 'y':
                    year = getNumber('y');
                    break;
...
var ms = $.spiceMobIScroll,
    ...
    var orig = $.extend({}, inst.settings),
        ...
        changeSz = [],
        getCustomWheels = function getCustomWheels(siteList) {
        var prevWheels,
            wheels = [{}],
    ...
    return {
        ...
        onBeforeShow: function onBeforeShow(dw) {
            $('input').blur();
        ...
        onSelect: function onSelect(v, inst) {},
        onChange: function onChange(v, inst) {},
...
var ms = $.spiceMobIScroll,
    ...
    return {
        ...
        onChange: function onChange(v, inst) {},
        validate: function validate(dw, i, time) {
            timer[i] = setTimeout(function () {}, time * 1000);
        },
        onClose: function onClose() {
            setTimeout(function () {
                $elem.blur();
...
function Scroller(elem, settings) {
    ...
    lock,
        ...
        onBtnStart = function onBtnStart(e) {
        var btn = $(this);
    ...
    function scrollToPos(time, index, manual, dir, active) {
        ...
        if (event('validate', [dw, index, time]) !== false) {
            ...
            v = s.formatResult(that.temp, that.settings.wheels);
            if (s.display == 'inline') {
    ...
    */
    that.show = function (prevAnim) {
        ...
        // Events
        $('.dwwl', dw).on('DOMMouseScroll mousewheel', onScroll).on(START_EVENT, onStart).on('keydown', onKeyDown).on('keyup', onKeyUp);
...
var move,
    ...
    speedUnit: 0.0012,
    timeUnit: 0.1,
    formatResult: function formatResult(d, w) {
        return d.join(' ');
    },
...
$(document).on('mouseover mouseup mousedown click', function (e) {
...
$.fn.spiceMobIScroll = function (method) {
    extend(this, $.spiceMobIScroll.shorts);
    return init(this, method, arguments);
};

$.spiceMobIScroll = $.spiceMobIScroll || {
    /**
    ...
    */
    ...
    presetShort: function presetShort(name) {
...
var fnList = ['position', 'enable', 'disable', 'setValue', 'getValue', 'getValues', 'changeWheel', 'isVisible', 'tap', 'show', 'hide', 'select', 'alert', 'cancel', 'init', 'trigger', 'option', 'destroy', 'getInst'],
    defaultOptions = {
	preset: 'list',
    ...
    defaultFunc = {
	funcList: function funcList(elem) {
		var o = {};
...
var MobIScroll = function MobIScroll(selector, options) {
